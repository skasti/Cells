...
{
    public class SkipIfNEMem: ICanUpdate
    {
        public class Maker : GeneMaker<SkipIfNEMem>
        {
            ...
            public override SkipIfNEMem Make(byte[] fragment)
            {
                if (fragment.Length < Size)
                    throw new GenomeTooShortException();

                return new SkipIfNEMem(
            ...
            public override byte[] MakeFragment(SkipIfNEMem gene)
            {
        ...
        public SkipIfNEMem(byte address, byte otherAddress, byte skipCount)
        {
            Address = address;
        ...
        public int Update(Organism self, float deltaTime)
        {
            ...
            else if (value.GetType() == other.GetType())
            {
                var ne = value != other;
            ...
            try {
                var ne = !Utils.IsEqual(value, other);
        ...
        {
            if (_string == null)
                _string = $"{Name} ([0x{Address:X2}] != [0x{Address:X2}]) SKIP {SkipCount}";
